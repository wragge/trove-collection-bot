...
def tweet_new():
    ...
    if authorised(request):
        ...
        print date_since
        # Dates only seemed to be searched by day -- so need to create a minimum of a day-long window
        date_to = '{}T00:00:00Z'.format(arrow.utcnow().shift(days=+1).format('YYYY-MM-DD'))
        url = 'http://api.trove.nla.gov.au/result/?q=nuc:"{}"+lastupdated:[{}+TO+{}]&zone={}&encoding=json&key={}'.format(NUC, date_since, date_to, ZONES, API_KEY)
        print url
...
def tweet_random():
    ...
    if authorised(request):
        ...
        url = 'http://api.trove.nla.gov.au/result/?q=nuc:"{}"&zone={}&encoding=json&n=1&s={}&key={}'.format(NUC, ZONES, start, API_KEY)
        print url
        response = requests.get(url)
        data = response.json()
        items = []
